...
int  cliptextready = 0;
int  cliptextlen   = 0;
char cliptext[512*1024];

void clip_sendto();
void * clip_thread(void * param);
...
static void initialize_xevents(int reset) {
	...
	if (reset) {
        ...
        //carwin: start thread check cliptext
        if(0==clipthreadflag)
        {
            pthread_t tid;
            pthread_create(&tid,NULL,clip_thread,NULL);
            clipthreadflag = 1;
        }
...
void check_xevents(int reset) {
	...
	if (now > last_call+1) {
		...
		int n = 0;
                ...
                //carwin: check cliptext and send
                clip_sendto();
...
void xcut_receive(char *text, int len, rfbClientPtr cl) {
    //carwin
    FILE * file = popen("DISPLAY=:0 xclip -sel clip","w");
    if(file) fwrite(text,1,len,file);
    if(file) pclose(file);
...
void clip_sendto()
{
    ...
    if(cliptextready)
    {
        rfbSendServerCutText(screen, cliptext, cliptextlen);
        cliptextready = 0;
    }
}
...
void * clip_thread(void * param)
{
    ...
    rfbLog("carwin: clip thread into\n");
    while(1)
    {
        usleep(500*1000);
        if(1==cliptextready) goto END;

        file = popen("DISPLAY=:0 xclip -sel clip -o 2>/dev/null","r");
        if(!file) goto END;

        len = fread(tmp,1,512*1024,file);
        if(len<=0) goto END;
        ...
        if(len==cliptextlen)
        {
            ...
            {
                for(i=0;i<len;i+=256)
                if(tmp[i]!=cliptext[i]){ sendflag=1; break; }
            }
            else
            {
                if(0!=memcmp(tmp,cliptext,len)) sendflag = 1;
            }
            if(0==sendflag) goto END;
        }

        //rfbLog("carwin: new cliptext\n");
        cliptextlen = len;
        memcpy(cliptext,tmp,len);
        cliptextready = 1;
